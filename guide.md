...
- Since TrueHD is lossless but has a lossy compatibility track, only apply `-keepDialnorm` to the lossy compatibility track.
  - `eac3to <BDMVFolderPath> <playlistNumber>) <trackNumber>: audio.ac3 -keepDialnorm audio.thd`
  - E.g. `eac3to "D:\discs\Tenet 2020 1080p Blu-ray AVC DTS-HD MA 5.1-ESiR" 1) 1:chapters.txt 2:video.* 3: audio.ac3 -keepDialnorm audio.thd 4:subtitles.* -log="eac3to.txt"`
- Verify that the eac3to log does not mention removal of dialogue normalization on any lossy track other than DTS-HD HRA.
...
- Include the best source for all unique isolated scores or music tracks you can find.
...
- Collect all unique subtitles from as many Blu-ray, DVD and streaming sources as you can find.
  - iTunes can be easily collected with [iTunes - subtitle downloader](https://greasyfork.org/en/scripts/422823-itunes-subtitle-downloader).
    - Use Google to search `inurl:tv.apple intext:"Title Name"` to find all regions the production is streaming in.
    - A button is added to the iTunes website to download the files in .VTT format.
  - Use the [Gnome Hut of Subs](https://example.com/forums/topics/3833) to find subtitles from various streaming services.
...
Using the sites below is encouraged in order to gain general, historical, and technical insight into the title you are preparing:
...
# Uncomment the next two lines if sources are out of sync. `Trim(100, 0)` for example trims off the first 100 frames. See http://avisynth.nl/index.php/Trim
#a=a.Trim(100, 0)
#b=b.Trim(100, 0)
...
Follow the [Subtitle OCR Guide](https://example.com/wikis/46) to convert low resolution VobSub image subtitles to text (only do this with languages in which you are literate). If the best available English subtitles/translation is from a DVD (VOBSub), you should OCR them to provide a more attractive option.
...
- If the codec is DTS-HD MA, DTS-HD HRA, DTS, DD, DD+, DD+ Atmos, FLAC or LPCM:
...
- If the codec is TrueHD:
  - Do NOT convert to DTS-HD MA.
    - Multichannel TrueHD streams contain alternative presentations (downmixes): 2.0, 5.1 and 7.1.
    - These are embeded either as discrete channels or derived through custom coefficients.
...
- Method 3:
  - Use BDSup2Sub programmatically:
    - Batch:
      ...
      ```
          for /f "tokens=*" %i in ('dir /b "*.sup"') do java -jar BDSup2Sub512.jar --delay "-1000" --fps-target "23.976" "%i" --output "synced/%i"
      ```
...
1\. PGS: en-US - For non-English dialogue 2. PGS: en-US - American 3. SRT: en-US - American (append \` / OCR\` or \` / Amazon\` to SRT's if needed) 4. PGS: en-GB - British 5. SRT: en-GB - British 6. PGS: en-US - SDH / American 7. SRT: en-US - SDH / American 8. PGS: en-GB - SDH / British 9. SRT: en-GB - SDH / British 10. PGS: ar - 11. PGS: zh-Hans - Simplified 12. PGS: zh-Hant - Cantonese / Traditional 13. PGS: zh-Hant - Traditional 14. PGS: nl - 15. SRT: nl - Netflix 16. PGS: fr-FR - 17. SRT: fr-FR- 18. PGS: fr-FR - SDH 19. SRT: fr-FR - SDH 20. PGS: fr-CA - Canadian 21. SRT: fr-CA - Canadian 22. PGS: fr-CA - SDH / Canadian 23. SRT: fr-CA - SDH / Canadian 24. PGS: por-PT - 25. PGS: por-BR - Brazilian 26. PGS: es-ES - 27. PGS: es-419 - Latin American 28. SRT: sv - 29. PGS: en-US - For non-English parts on English dub 30. PGS: en-US - Dub 31. PGS: en-US - Commentary by ... 32. PGS: fr-FR - Commentary by ... 33. PGS: fr-CA - Canadian / Commentary by ...
...
- Dialects:
  ...
  - Other dialects should be named based on the following tags and names:
    - Chinese (Simplified) - zh-Hans - Simplified
    - Chinese (Traditional) - zh-Hant - Cantonese / Traditional
    - Chinese (Traditional) - zh-Hant - Traditional
    - English - en-US - American (Used when it's American English and only named if both dialects are present)
    - English - en-GB - British (Used when it's British English and only named if both dialects are present)
    - French - fr-FR (Used when it's European)
    - French - fr-CA - Canadian
    - Portuguese - por-PT (Used when it's European)
    - Portuguese - por-BR - Brazilian
    - Spanish - es-ES (Used when it's Castilian/European)
...
- Intertitles in the production's primary language only if there are no other forced subtitle tracks in the same language.
...
- 2025-06-16: Impoved subtitle ordering example.
